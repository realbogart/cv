...
if len(sys.argv) < 3:
    print("Usage: python convert-playwright.py <port> <output_file_path> [json_file]")
    sys.exit(1)

port = int(sys.argv[1])
output_file_path = sys.argv[2]
json_file = sys.argv[3] if len(sys.argv) > 3 else 'data.json'
...
def run_browser(playwright, port, output_file_path, json_file):
    with playwright.chromium.launch(executable_path=shutil.which("chromium") or 'chromium') as browser:
        page = browser.new_page()
        url = f'http://localhost:{port}?data={json_file}'
        page.goto(url)
...
def main():
    ...
    with sync_playwright() as playwright:
        run_browser(playwright, port, output_file_path, json_file)
